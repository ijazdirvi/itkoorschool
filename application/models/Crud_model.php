...
class Crud_model extends CI_Model {
    ...
    /////////WITHDRAW STUDENTS////////////
    function get_withraw_students($student_id) {

        $query = $this->db->get_where('student_withdraw', array('student_id' => $student_id));

        return $query->result_array();
    }

    function get_invoice_payment($class_id, $running_year) {

        //$id= (int) $class_id;

        $this->db->select('enroll.roll,student.name,payment.title,invoice.invoice_id,invoice.due,invoice.amount,invoice.amount_paid,invoice.status,invoice.creation_timestamp');

        $this->db->from('payment');

        $this->db->join('student', 'student.student_id = payment.student_id', 'inner')->distinct();

        $this->db->join('enroll', 'enroll.student_id = payment.student_id', 'inner')->distinct();

        $this->db->join('invoice', 'invoice.student_id = payment.student_id', 'inner')->distinct();

        $this->db->where(array('enroll.class_id' => $class_id, 'enroll.year' => $running_year));
    ...
    /*
     * convert the amount in number to words according 
     */
