...
function student_transaction($param1 = '', $param2 = '') {
    ...
    $page_data['student_transaction'] = $this->db
            ->group_by('student_id')
            ->get_where('student_transaction', array('session' => $year , 'student_id'=>$param1))
            ->result_array();
...
function transaction_detail($param1 = '', $param2 = '') {
    ...
    $page_data['student_id'] = $param1;
    $class_id = $this->db->get_where('enroll', array('student_id'=>$param1))->row()->class_id;
    $page_data['class_id'] = $class_id;
    $page_data['class_id2'] = '';

    $page_data['page_name'] = 'transaction_detail';
    $page_data['page_title'] = get_phrase('transaction_detail');
    $this->load->view('backend/index', $page_data);
...
function attendance_report_selector() {
    ...
    redirect(base_url() . 'index.php?parents/attendance_report_view/' . $data['class_id'] . '/' . $data['section_id'] . '/' . $data['month'], 'refresh');
}
